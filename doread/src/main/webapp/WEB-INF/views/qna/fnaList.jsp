...
<div class="page-main" id="up">
	...
	<div class="content-main">
		...
		<div class="list-space align-right list-btn2">
			...
			<c:if test="${user_auth == 9}">
			<input type="button" value="QnA목록" onclick="location.href='qnaList.do'">
			</c:if>
		...
		<table>
			...
			<td>
				...
				주문하신 상품이 배송준비중 전일 경우, 직접 배송지 주소를 변경하실 수 있습니다.<br>
			...
			<td id="pointfaq">
				...
				적립금은 구매금액의 3% 적립되며, 배송비와 적립금 사용금액은 제외됩니다.<br>
				상품 주문 시 적립 예정 포인트가 자동 적립됩니다.
			 	<br>
			 	</div>
		...
		<div class="hide divv" id="adddiv">
			...
			<td>
				<h4> Q[배송]주문 상품이 언제 출고 되는지 알 수 있나요?</h4>
			 	<div>
				각 상품별로 출고예정일이 상이하며, 각 상품별 출고예정일은 QNA게시판에 문의해주세요. <br>
				출고예정일이란, 주문상품의 결제(무통장입금의 경우 입금확인)가 확인된 날 기준으로 상품을 준비하여 상품 포장 후<br> 물류센터에서 택배사로 전달하게 되는 예상 일자입니다.<br>
				배송일정은 예상출고일에 택배사의 배송일(약 2일)이 더해진 날이며 연휴 및 토,일,공휴일을 제외한 근무일 기준입니다. <br>
				다양한 상품을 함께 주문하실 경우 가장 늦은 출고일정에 맞춰 함께 배송됩니다.<br>
				*출고예정일이 5일 이상인 상품의 경우, 출판사/유통사 사정으로 품/절판 되어 구입이 어려울 수 있습니다.<br> 이 경우 SMS,메일로 알려드립니다. 고객님께서 급하게 필요하신 상품은 별도로 주문하면 받으시는 시간이 절약됩니다.<br> 
			...
			<td id="pfaq">
				...
				1.고객님의 변심 또는 주문오류로 인한 반품신청일 때, 타 도서로의 교환이 불가하며,<br>
				DoRead에서 주문하신 상품의 경우, 반품 교환이 가능한 종류에 한해 상품을 수령하신 날로부터 7일 이내 반품신청이 가능합니다.<br>	
				반송료는 고객님께서 부담하시게 됩니다. <br>
				단, 반품하실 도서는 사용하지 않고, 상품 자체의 비닐 래핑을 벗기지 않은 상태일 때 가능합니다.<br>
				...
				혹시 반품신청서가 없으시면 메모지에 주문번호와 반송사유를 간략하게 기입하여 주십시오.<br>
			 	</div>
