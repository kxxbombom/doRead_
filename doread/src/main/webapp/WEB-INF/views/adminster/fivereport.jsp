...
<h2>USED 보드 신고된 게시글 관리</h2>
	...
	<c:if test="${!empty list}">
	 <table>
	 	...
	 	<c:if test="${i.auth == 0 && c.number ==i.u_num && c.sumcount >=5 && c.type =='u_num'}">
	 	<tr>
	...
	<c:if test="${!empty list1}">
	 <table>
	 	...
	 	<c:forEach var="i" items="${list1}">
	 		...
	 		<c:if test="${i.auth == 0 && i.uc_num ==c.number && c.sumcount >=5 && c.type =='uc_num'}">
	...
	<c:if test="${!empty list2}">
	 <table>
	 	...
	 	<c:forEach var="i" items="${list2}">
	 		...
	 		<c:if test="${i.auth == 0 && i.s_num ==c.number && c.sumcount >=5 && c.type =='s_num'}">
	...
	<c:if test="${!empty list3}">
	 <table>
	 	...
	 	<c:forEach var="i" items="${list3}">
	 		...
	 		<c:if test="${i.auth == 0 && i.sc_num ==c.number && c.sumcount >=5 && c.type =='sc_num'}">
	 		<tr>
	 			<td><a href="${pageContext.request.contextPath}/story/storyDetail.do?s_num=${i.s_num}">${i.s_num}</a></td>
	 			<td>${i.sc_num}</td>
	 			<td>${i.content}</td>
