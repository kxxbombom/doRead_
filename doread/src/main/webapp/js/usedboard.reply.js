  $(function(){  
	...
	function selectList(pageNum){
		...
		//서버와 통신
		$.ajax({
			url:'listUBComment.do',
			type:'post',
			data:{pageNum:pageNum,rowCount:rowCount,u_num:$('#u_num').val()},
			dataType:'json',
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					if(param.user_num == item.mem_num){
						output += ' <input type="button" data-renum="'+item.uc_num+'" value="수정" class="modify-btn">';
						output += ' <input type="button" data-renum="'+item.uc_num+'" value="삭제" class="delete-btn">';
					}
	 ...
	 *=======================================================*/
	//댓글 등록
	$('#uc_form').submit(function(event){
		if($('#uc_content').val().trim()==''){
			alert('내용을 입력하세요');
			$('#uc_content').val('').focus();
			return false;
		}
	...
	$(document).on('click','.modify-btn',function(){
		//댓글 번호
		let uc_num = $(this).attr('data-renum');
		//댓글 내용
		let content = $(this).parent().find('p').html().replace(/<br>/gi,'\n');
																//g: 지정문자열 전부,i:대소문자 무시
		//댓글 수정폼 UI
		let modifyUI = '<form id="mre_form">'
		modifyUI += '<input type="hidden" name="uc_num" id="mre_num" value="'+uc_num+'">';
		modifyUI += '<textarea rows="3" cols="50" name="uc_content" id ="mre_content" class="rep-content">'+content+'</textarea>';
